...
Server.prototype.tokenMiddleware = function tokenMiddleware(req, res, next) {
  ...
  this.db.get(this.dbKey('token', token), function(err, user) {
    if (err || !user)
...
Server.prototype.handleLogout = function handleLogout(req, res) {
  var dbKey = this.dbKey('token', req.token);
    ...
    async.map(domains, function(domain, callback) {
      self.getSubdomains(domain, function(err, subdomains) {
        ...
        async.map(subdomains, function(subdomain, callback) {
          self.getDomainRecords(req.user, subdomain, function(err, records) {
            if (err)
              return callback(err);
            callback(null, { subdomain: subdomain, records: records });
          });
        }, callback);
      });
...
Server.prototype.handleGetDomainRecords =
  ...
  var self = this;
  this.getSubdomains(req.params.domain, function(err, subdomains) {
    if (err)
...
Server.prototype.handleRemoveRecords = function handleRemoveRecords(req, res) {
  ...
  this.validateRecords(req.user, req.body.records, res, function() {
    ...
    fetch.exec(function(err, results) {
      if (err)
        return res.json(500, { error: 'Failed to fetch records' });

      var rm = self.db.multi();
      results.forEach(function(result, i) {
        var record = req.body.records[i];
        var match = false;
        result.some(function(rawEntry) {
          try {
        ...
        rm.srem(key, rawEntry);

        if (results.length === 1) {
          // The only one record for this subdomain
          var subKey = self.dbKey('domain',
                                 'sub',
                                  utils.tld(record.domain.toLowerCase()));
          rm.srem(subKey, record.domain.toLowerCase());
        }
      });
